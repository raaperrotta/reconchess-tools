...
class MhtBot(Player):
    ...
    def handle_opponent_move_result(
        ...
        # Drop boards where the game would be over
        self.mht.boards = [b for b in self.mht.boards if b.king(b.turn) is not None]
    ...
    def choose_move(
        ...
        move = self.vote(move_actions, self.mht.boards)
    ...
    def handle_move_result(
        ...
        self.mht.move(
            ...
            capture_square,
            include_king_captures=False,  # drop boards for which the game would be over
        )
        # Drop boards where the game would be over
        self.mht.boards = [b for b in self.mht.boards if b.king(b.turn) is not None]
    ...
    def vote(self, possible_requested_moves, boards):
        ...
        for board in tqdm(boards[:1200]):
            ...
            if king_attackers:
                ...
                for attacker in king_attackers:
                    ...
                    # Add promotion to queen if possible
                    taken_move = add_pawn_queen_promotion(board, taken_move)
                ...
                try:
                    results = self.engine.analyse(
                        board, limit=chess.engine.Limit(depth=11), multipv=3
                    )
                except chess.engine.EngineTerminatedError:
                    print(f"Stockfish died while analyzing {board.epd()}! \n")
                    print("  a b c d e f g h")
                    print("\n".join(f"{i+1} {line} {i+1}" for i, line in enumerate(board.unicode(invert_color=True, empty_square=".").splitlines())))
                    print("  a b c d e f g h")
                    print(f"{chess.COLOR_NAMES[board.turn]} to move")
                    print("Will restart engine and retry one time")
                    self.engine = self.create_engine()
                    try:
                        results = self.engine.analyse(
                            board, limit=chess.engine.Limit(depth=11), multipv=3
                        ...
                        print(f"Stockfish died while retrying board analysis!")
                        raise
            ...
            # Then rank as tied all moves that don't put us in check followed by moves that do.
            # This additionally discourage moves into check (besides just not ranking them highly).
            into_check = []
            meh = []
            already_counted = {move for group in my_ranked_votes for move in group}
            for taken_move, requested_moves in move_lookup.items():
                if board.is_into_check(taken_move):
                    into_check += list(set(requested_moves) - already_counted)
                else:
                    meh += list(set(requested_moves) - already_counted)
            if meh:
                my_ranked_votes.append(meh)
            if into_check:
                my_ranked_votes.append(into_check)
            assert {move for group in my_ranked_votes for move in group} == set(possible_requested_moves)
